...
int getLastPatientID()
{
    ...
    int id;
    int lastID = -1;
    char name[200], addr[300];
    long long phone;

    while (fscanf(fp, "%d|%199[^|]|%299[^|]|%lld\n", &id, name, addr, &phone) == 4)
    {
        lastID = id;
    }
...
void addpatient()
{
    ...
    struct patient p;
    int lastID = getLastPatientID();
    p.patient_id = lastID + 1;
    printf("Enter patient NAME:");
    getchar();
    fgets(p.name, 100, stdin);
    printf("Enter Address:");
    fgets(p.address, 300, stdin);
    temp = p.phone_no;
    while (count != 10)
    {
    ...
    fclose(fp);
    printf("Patient ID:%d", p.patient_id);
